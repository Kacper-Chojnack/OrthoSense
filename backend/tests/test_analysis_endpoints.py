...
def test_get_status():
    ...
    with patch("app.api.v1.endpoints.analysis.is_ai_available", return_value=True):
        with patch("app.api.v1.endpoints.analysis.get_ai_system") as mock_get:
...
def test_websocket_rejected_when_disabled():
    ...
    try:
        with client.websocket_connect(f"{settings.api_v1_prefix}/analysis/ws/test_client") as websocket:
