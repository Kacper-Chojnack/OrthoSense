...
def draw_overlay(frame, text, subtext="", color=(0, 255, 0)):
    ...
    if subtext:
        text_y = h // 2 - 10
        (sub_w, _), _ = cv2.getTextSize(subtext, font, 0.8, 2)
...
def main():
    ...
    while cap.isOpened():
        ...
        world_landmarks, image_landmarks = system.processor.process_frame(frame_rgb)
        
        if world_landmarks is not None:
            raw_buffer.append(world_landmarks)
            ...
            if len(raw_buffer) == WINDOW_SIZE and frame_count % PREDICTION_INTERVAL == 0:
        ...
        if phase == "SETUP":
            ...
            if locked_exercise is None:
                cv2.putText(frame, "No exercise detected", (20, 60), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (100, 100, 100), 2)
            else:
                ...
                if current_feedback['is_correct']:
                    ...
                    cv2.putText(frame, f"FIX: {clean_msg}", (20, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
