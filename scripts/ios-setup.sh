...
if [ -f "$CONFIG_FILE" ]; then
    if [ "$INTERACTIVE_MODE" -eq 1 ]; then
        echo "⚠️  Developer.xcconfig already exists."
...
if [ ! -z "$TEAM_IDS" ]; then
    ...
    echo "✅ Found Team ID: $DETECTED_TEAM_ID"
fi

# Get Team ID (with auto-detected as default)
echo ""
echo "📋 Step 1: Apple Development Team ID"
echo ""
if [ -z "$DETECTED_TEAM_ID" ]; then
    if [ "$INTERACTIVE_MODE" -eq 1 ]; then
        echo "ℹ️  Could not auto-detect Team ID."
        echo "Find your Team ID in Xcode → Settings (⌘,) → Accounts → Your Team"
        echo ""
    ...
    if [ "$INTERACTIVE_MODE" -eq 1 ]; then
        ...
        echo "✅ Using detected Team ID: $TEAM_ID"
    fi
fi
...
if ! [[ "$TEAM_ID" =~ ^[A-Z0-9]{10}$ ]]; then
    if [ "$INTERACTIVE_MODE" -eq 1 ]; then
        echo "⚠️  Warning: Team ID format seems unusual (expected 10 uppercase alphanumeric characters)"
        ...
        echo "⚠️  Warning: Team ID format seems unusual, but continuing..."
    fi
fi

# Auto-generate Bundle ID
echo ""
echo "📦 Step 2: Bundle Identifier"
echo ""
...
if [ "$INTERACTIVE_MODE" -eq 1 ]; then
    ...
    echo "✅ Using generated Bundle ID: $BUNDLE_ID"
fi
...
if ! [[ "$BUNDLE_ID" =~ ^[a-zA-Z0-9.-]+$ ]]; then
    echo "❌ Error: Invalid Bundle ID format (should contain only letters, numbers, dots, and hyphens)"
    exit 1
fi

# Create configuration file
echo ""
echo "🔧 Creating Developer.xcconfig..."
...
// Your Apple Development Team ID (find it in Xcode → Settings → Accounts → Your Team)
...
echo "  2. Clean extended attributes: xattr -cr ."
echo "  3. Run: flutter pub get"
echo "  4. Run: flutter run --release"
echo ""
echo "ℹ️  If you encounter codesigning errors, run:"
echo "  find . -name '*.framework' -exec xattr -cr {} \\;"
echo ""
echo "✨ Setup complete!"
